...
const SERVER_BUILD_TAG = "2026-03-07-usermenu-audio-r3";
...
function buildRudiConfig() {
  ...
  for (const entry of Array.from(knownServiceEntries.values())) {
...
function resolveResourcePath(servicePath, requestPath) {
  if (String(servicePath || '').toLowerCase() === '/targets' || String(servicePath || '').toLowerCase() === '/persistenceentries') {
    console.log('[probe.resolveResourcePath.enter]', JSON.stringify({ servicePath, requestPath }));
  }
  ...
  if (directAlias) {
    ...
    const resolvedAliasPath = normalizePath(`${normalizedService}${aliasPrefix}${normalizedRequestPath}`);
    if (String(servicePath || '').toLowerCase() === '/targets' || String(servicePath || '').toLowerCase() === '/persistenceentries') {
      console.log('[probe.resolveResourcePath.alias]', JSON.stringify({ servicePath, normalizedRequestPath, resolvedAliasPath }));
    }
...
function handleActionRequest(ws, message) {
  ...
  try {
    ...
    } else if (service === "/registry" && command === "insert") {
      const requestedPath =
    ...
    } else if (["update", "insert", "delete"].includes(command)) {
      responseBody = resolveMutationBody(command, service, fullPath, requestBody);
      if (command !== "delete") {
        responseBody = coerceMutationAck(command, fullPath, responseBody);
      }
    } else {
